<?php
class bv30v_action extends bv30v_base {
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv30v_view ( $this->application () );
		}
	}
	public function updated($message = 'Settings Saved') {
		$return = '';
		if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
			$this->view->class = 'normal';
			$this->view->slug = 'updated';
	...
	private function get_action_meta($method, $meta) {
		...
		if ($meta ['title'] === null || $meta ['title'] == '' || ($this->settings ()->application ['probono'] == 0 && $meta ['probono'] == true)) {
			return false;
		}
		return $meta;
	}
	...
	protected function add_action_type($action_type, $tag = null, $default_meta = array()) {
		...
		$meta ['probono'] = false;
		$meta = bv30v_data_xml::merge_replace_recursive ( $meta, $default_meta );
		$meta ['tag'] = $tag;
