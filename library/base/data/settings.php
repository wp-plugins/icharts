<?php
class bv35v_data_settings extends bv35v_base {
	protected $_option;
	...
	private static function load($file, $legacy = false) {
		...
		if (file_exists ( $file )) {
			switch (pathinfo ( $file, PATHINFO_EXTENSION )) {
				case 'xml' :
					$return = bv35v_data_xml::load ( $file );
					break;
				case 'json' :
					$return = bv35v_data_json::decode ( file_get_contents ( $file ), $legacy );
					break;
			}
		}
		return $return;
	...
	public function refresh() {
		...
		if (null !== $this->_option) {
			$base = new bv35v_data_settings ( $this->application () );
			$settings [] = $base->data ();
		...
		$this->_data = bv35v_data_array::merge ( $settings );
		return $this->_data;
	}
