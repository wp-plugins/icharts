<?php
class wv30v_action extends bv30v_action {
	public function SupportActionMeta($return) {
		if (! isset ( $this->settings ()->application ['wordpress'] ['wpslug'] )) {
			$return ['hide'] = 1;
		} else {
			$return ['probono'] = true;
			$return ['title'] = 'Support Forum';
			$return ['url'] = "http://wordpress.org/tags/" . $this->settings ()->application ['wordpress'] ['wpslug'] . "?forum_id=10";
			$return ['classes'] [] = 'v30v_forum';
			$return ['priority'] = 10;
		}
		return $return;
	}
	public function PluginActionMeta($return) {
		if (! isset ( $this->settings ()->application ['wordpress'] ['uri'] )) {
			$return ['hide'] = 1;
		} else {
			$return ['probono'] = true;
			$return ['title'] = 'Plugin Site';
			$return ['url'] = $this->settings ()->application ['wordpress'] ['uri'];
			$return ['classes'] [] = 'v30v_home';
			$return ['priority'] = 10;
		}
		return $return;
	}
	public function DonateActionMeta($return) {
		if (! isset ( $this->settings ()->application ['wordpress'] ['donate_link'] )) {
			$return ['hide'] = 1;
		} else {
			$return ['link_name'] = $return ['title'];
			$return ['probono'] = true;
			$return ['url'] = $this->settings ()->application ['wordpress'] ['donate_link'];
			$return ['classes'] [] = 'v30v_donate';
			$return ['priority'] = 10;
		}
		return $return;
	}
	...
	protected function set_view() {
		$this->view = new wv30v_view ( $this->application () );
	}
//	public function control_url($page) {
//		$return = rtrim ( get_option ( 'siteurl' ), '/' );
//		$page = ltrim ( $page, '/' );
//		if (get_option ( 'permalink_structure' ) == '') {
//			$return .= '/index.php?wppage=';
//		} else {
//			$return .= '/';
//		}
//		$return .= $page;
...
//	}
	 ...
	 * Setup Aciton Types
	 *******************************************************************/
	private function setup_wpactions() {
	...
	public function callback() {
		...
		if ($menu) {
			...
			$output = "<form  id='v30v_form' action='' method='post' class='admin-menu {$classes}'>{$updated}{$output}</form>";
	...
	public function wrapper($page, $classes = array(), $attr = null, $tag = 'div') {
		$classes [] = 'wrap';
		$classes [] = 'v30v';
		$classes [] = 'v30v_' . $this->settings ()->application ['slug'];
		...
		foreach ( $this->view->items as $key => $value ) {
			if ($value ['hide']) {
				...
				if ((! isset ( $_GET ['page2'] ) && ! $current) || substr ( $_SERVER ['REQUEST_URI'], - strlen ( $this->view->items [$key] ['url'] ) ) == $this->view->items [$key] ['url']) {
					$this->view->items [$key] ['classes'] [] = 'v30v_current';
					$current = true;
				}
